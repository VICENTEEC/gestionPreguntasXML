...
public class PreguntaListaAssembler implements RepresentationModelAssembler<Pregunta, PreguntaListaModel> {
	...
	public PreguntaListaModel toModel(Pregunta entity) {
			...
			model.add(
					...
					linkTo(methodOn(UsuarioController.class).one(entity.getUsuario().getId())).withRel("usuariodesdePreguntListAssembler")
					);
			return model;
	}
	...
	public CollectionModel<PreguntaListaModel> toCollection(List<Pregunta> lista) {
		...
		collection.add(
				linkTo(methodOn(PreguntaController.class).all()).withRel("preguntasdesdePreguntListAssembler")
				);
		return collection;
	}
}
